...
v_tot <- d_vacc_status %>% 
  filter(transition %in% c(2,9,12)) %>% 
  count(transition)
d_tot <- d_vacc_status %>% 
  filter(state_to == "death") %>% 
  count(state_to)
i_tot <- d_vacc_status %>% filter(state_to == "infection") %>% 
  dplyr::select(alf_e, state_to) %>% 
  distinct() %>% 
  count(state_to)
d_sum <- d_vacc_status%>% 
...
t_sex <- d_vacc_status %>% 
  ...
  rename(type = sex)
t_age <- d_vacc_status %>% 
  distinct(alf_e, age_cat) %>%
  group_by(age_cat) %>% 
  summarise(n = n()) %>% 
  rename(type = age_cat)
t_hhn <- d_vacc_status %>% 
  distinct(alf_e, household_n) %>%
  group_by(household_n) %>% 
  summarise(n = n()) %>% 
  rename(type = household_n)
t_hhv <- d_vacc_status %>% 
  distinct(alf_e, hh_vaccinated) %>%
  group_by(hh_vaccinated) %>% 
  summarise(n = n()) %>% 
  rename(type = hh_vaccinated)
...
#ms_vacc_all<-filter(ms_vacc_all, !(age_cat=="05_11" & trans==12))
#ms_vacc_all<-filter(ms_vacc_all, !(age_cat=="05_11" & trans==13))

# define reference groups
ms_vacc_all$age_cat <- factor(ms_vacc_all$age_cat, 
                              levels = c("16_17", "12_15", "05_11"))
ms_vacc_all$hh_vaccinated <- factor(ms_vacc_all$hh_vaccinated, 
                                    levels = c("pv", "uv", "fv"))
ms_vacc_all$household_n <- factor(ms_vacc_all$household_n, 
                                  levels = c("3", "2", "4", "5+"))
ms_vacc_all$sex <- factor(ms_vacc_all$sex, 
                          levels = c("F", "M"))

key_values <- ms_vacc_all %>% 
  filter(tstart == "0") %>% 
  dplyr::select(id, from) %>% 
  mutate(
...
# )
saveRDS(ms_vacc_all, "d_ms_vacc_all.rds")

# qsave(
#   key_values,
#   file = ("results/key_values.qs")
# )
saveRDS(key_values, "results/key_values.rds")

# qsave(
#   d_sum,
#   file = ("results/d_sum.qs")
# )
saveRDS(d_sum, "results/d_sum.rds")

# qsave(
#   p_sum,
#   file = ("results/p_sum.qs")
# )
saveRDS(p_sum, "results/p_sum.rds")

# qsave(
#   t_sum,
#   file = ("results/t_sum.qs")
# )
saveRDS(t_sum, "results/t_sum.rds")
