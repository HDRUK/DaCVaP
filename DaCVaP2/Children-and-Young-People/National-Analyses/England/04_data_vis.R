...
c1 <- readRDS("~/DaCVaP CYP/results/c1.rds")
ms_vacc_a <- readRDS("~/DaCVaP CYP/ms_vacc_a.rds")
...
newd$sex <- factor(newd$sex, levels = 0:1, labels = c("F", "M"))
...
newd2 <- newd1
newd2$sex <- 1
newd2$sex <- factor(newd2$sex, levels = 0:1, labels = c("F","M"))
...
ch1 <- data.frame(time = summ.msf.f[[2]]$time, cov = "age", group = "F", transition = "unvacc -> vacc1", 
                  surv = survf2, low = lowf2, upp = uppf2) %>%
  add_row(time = summ.msf.m[[2]]$time, cov = "age", group = "M", transition = "unvacc -> vacc1", 
          surv = survm2, low = lowm2, upp = uppm2) %>%
  add_row(time = summ.msf.f[[9]]$time, cov = "age", group = "F", transition = "vacc1 -> vacc2", 
          surv = survf9, low = lowf9, upp = uppf9) %>%
  add_row(time = summ.msf.m[[9]]$time, cov = "age", group = "M", transition = "vacc1 -> vacc2", 
          surv = survm9, low = lowm9, upp = uppm9) %>%
  add_row(time = summ.msf.f[[12]]$time, cov = "age", group = "F", transition = "vacc2 -> vacc3", 
          surv = survf12, low = lowf12, upp = uppf12) %>%
  add_row(time = summ.msf.m[[12]]$time, cov = "age", group = "M", transition = "vacc2 -> vacc3", 
...
#group_colors=c("5-11"="#619CFF", "12-15"="#F8766D", "16-17"="#00BA38")
...
# )
saveRDS(pta, "results/pta.rds")

# qsave(
#   p_ch1,
#   file = ("results/p_ch1.qs")
# )
saveRDS(p_ch1, "results/p_ch1.rds")

# qsave(
#   ch1,
#   file = ("results/t_ch1.qs")
# )
saveRDS(ch1, "results/t_ch1.rds")

# qsave(
#   p_ch2,
#   file = ("results/p_ch2.qs")
# )
saveRDS(p_ch2, "results/p_ch2.rds")

# qsave(
#   ch2,
#   file = ("results/t_ch2.qs")
# )
saveRDS(ch2, "results/t_ch2.rds")

# qsave(
#   p_ch3,
#   file = ("results/p_ch3.qs")
# )
saveRDS(p_ch3, "results/p_ch3.rds")

# qsave(
#   ch3,
#   file = ("results/t_ch3.qs")
# )
saveRDS(ch3, "results/t_ch3.rds")

# qsave(
#   p_ch4,
#   file = ("results/p_ch4.qs")
# )
saveRDS(p_ch4, "results/p_ch4.rds")

# qsave(
#   ch4,
#   file = ("results/t_ch4.qs")
# )
saveRDS(ch4, "results/t_ch4.rds")
