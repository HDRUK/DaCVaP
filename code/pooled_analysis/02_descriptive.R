...
# Add indicators for any hospitalisation or death in cohort period

hosp_IDs <-filter(all_hospitalisations, admission_date <= a_end & admission_date >= a_begin) %>%
                    pull(EAVE_LINKNO) %>%
                    unique()

z_chrt_desc <- mutate(z_chrt_desc, any_hosp = case_when(EAVE_LINKNO_uv %in% hosp_IDs ~ 1,
                                                        TRUE ~ 0) )

death_IDs <- filter(any_death, admission_date <= a_end & admission_date >= a_begin) %>%
                    pull(EAVE_LINKNO) %>%
                    unique()

z_chrt_desc <- mutate(z_chrt_desc, any_death = case_when(EAVE_LINKNO_uv %in% death_IDs ~ 1,
                                                        TRUE ~ 0) )
...
explanatory <- c("Total", "any_hosp", "any_death", "Sex", "ageYear", "age_grp", "simd2020_sc_quintile", "ur6_2016_name", "n_risk_gps",
...
summary_tbl_wt1 <- summary_factorlist_wt(z_chrt_desc %>%
                                           mutate_at(vars(qcovid_diags, 'any_hosp', 'any_death'), function(x) as.character(x)) %>% 
                                           mutate(care_home_elderly = as.character(care_home_elderly)),
                                         "vacc_type", explanatory = explanatory)

summary_tbl_wt1 <- select(summary_tbl_wt1, 'characteristic', 'levels', 'NA',
                          'AZ', 'PB')

names(summary_tbl_wt1) <- c('Characteristic', 'Levels', 'Unvaccinated', 'ChAdOx1', 'BNT162b2')
...
write.csv(summary_tbl_wt1, "./output/summary_table_weights.csv", row.names = F)


# Event counts and rates table
event_summary_tbl_wt <- event_summary_wt(z_chrt_desc)

write.csv(event_summary_tbl_wt, "./output/event_summary_table_weights.csv", row.names = F)
...
# Combine and save
#summary_tbl_wt <- left_join(summary_tbl_wt1, summary_tbl_wt2)
write.csv(summary_tbl_wt2, "./output/summary_table_non_carehome_weights.csv", row.names=F)

# Event counts and rates table
event_summary_tbl_wt <- event_summary_wt(filter(z_chrt_desc, care_home_elderly == 0))

write.csv(event_summary_tbl_wt, "./output/event_summary_table_non_carehome_weights.csv", row.names = F)
